import 'dart:convert';
import 'package:app_cat_breeds/data/dtos/weight_cat_breeds_dto.dart';

CatBreedDto catBreedDtoFromJson(String str) => CatBreedDto.fromJson(json.decode(str));

String catBreedDtoToJson(CatBreedDto data) => json.encode(data.toJson());

class CatBreedDto {
  final WeightCatBreedDto weight;
  final String id;
  final String name;
  final String cfaUrl;
  ...
  factory CatBreedDto.fromJson(Map<String, dynamic> json) => CatBreedDto(
        weight: WeightCatBreedDto.fromJson(json["weight"]),
        id: json["id"] ?? '',
        name: json["name"] ?? '',
        cfaUrl: json["cfa_url"] ?? '',
